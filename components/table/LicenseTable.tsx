...
export default function LicenseTable() {
    ...
    const table = useMaterialReactTable({
        ...
        getRowId: (row) => row.id,
        // 컨테이너(스크롤 박스) — 내부 스크롤 유지
        // 다크모드에서만 OddsJam 톤 적용 (라이트는 손대지 않음)
        muiTablePaperProps: {
            sx: (theme) =>
        ...
        // 컨테이너(내부 스크롤) — 스크롤 자체는 공통, 다크 전용 디테일만 조건부
        ...
        onCreatingRowSave: async ({ values, row, table }) => {
            ...
            try {
                ...
                // 기본 유효성 검사
                if (!merged.userId || !merged.keyName) {
                ...
                // introducer 처리 (선택)
                const introducerEmail = (merged.introducerEmail ?? "").trim();
                let introducerId: string | null = null;

                if (introducerEmail) {
                    const user = users.find((u) => u.id === merged.userId);
                    if (user?.email === introducerEmail) {
                        showSnackbar("Introducer email cannot be your own.", "warning");
                        return;
                    }
                    const introducer = users.find((u) => u.email === introducerEmail);
                    if (!introducer) {
                        showSnackbar("Introducer email does not exist.", "error");
                        return;
                    }
                ...
                const payload = {
                    ...merged,
                    introducerId,
                    startTime: safeToISOString(merged.startTime),
                    endTime: safeToISOString(merged.endTime),
                    lastUsedTime: safeToISOString(merged.lastUsedTime),
                    usePeriod: Number(merged.usePeriod ?? 0) || 0,
                    price: Number(merged.price ?? 0) || 0,
                    isBlocked: !!merged.isBlocked,
                    isRunning: !!merged.isRunning,
                };
