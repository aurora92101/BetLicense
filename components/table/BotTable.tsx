...
export default function BotTable({ isAdmin = false }: any) {
    ...
    const isXs = useMediaQuery(theme.breakpoints.down("sm")); // 모바일 판단
                ...
                Cell: ({ cell, row }) => {
                    ...
                    return (
                        <CopyableFileCell
                            ...
                            isAdmin={isAdmin}
                            label={
                                isAdmin ? undefined : pretty || url.split("/").pop() || "Download"
                            }
                        />
                    );
                },
    ...
    const table = useMaterialReactTable({
        ...
        // ✅ 모바일에서만 상세패널(Expand) 활성화 + 파일 다운로드 제공
        enableExpanding: isXs,
        renderDetailPanel:
            isXs
                ? ({ row }) => <MobileDetailPanel row={row} />
                : undefined,
            ...
            ? ({ row, table }) => (
                <Box sx={{ display: "flex", gap: "0.5rem" }}>
                    ...
                    <Tooltip title="Upload File">
                        <IconButton component="label" color="primary" sx={{ cursor: "pointer" }}>
                            <CloudUploadIcon />
                            <input
                                type="file"
                                hidden
                    ...
                    <Tooltip title="IsAutoUpdate">
                        <Switch
                            ...
                            onChange={async (e) => {
                                ...
                                await mutate("/api/bookie");
                                showSnackbar(
                                    "AutoUpdating Station changed successfully!",
                                    "warning"
                                );
                            }}
                        />
...
function MobileDetailPanel({ row }: { row: MRT_Row<Bookie> }) {
    const r = row.original;
    const url = r.botFileUrl || "";
    const ext = url.match(/\.([a-z0-9]+)$/i)?.[0] || "";
    const pretty =
        (r.bookieName && r.botVersion && `${r.bookieName}Bot-${r.botVersion}${ext}`) ||
        url.split("/").pop() ||
        "download";
    ...
    return (
        ...
        >
            ...
            <DetailItem label="Version">{r.botVersion || "—"}</DetailItem>
            <DetailItem label="Uploaded At">
...
// Typography 중첩 문제 없이 label/value를 정갈하게 출력
function DetailItem({
    label,
    children,
}: {
    ...
    return (
        <Box sx={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 0.75 }}>
            <Typography sx={{ color: "text.secondary", fontSize: 12, lineHeight: 1.3 }}>
                {label}
            </Typography>
            <Typography component="div" sx={{ fontSize: 14, lineHeight: 1.4, wordBreak: "break-word" }}>
                {children}
            </Typography>
        </Box>
    );
